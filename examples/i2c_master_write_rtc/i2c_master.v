 ...
 * Date: 4/12/2020
 * Edited by: Hammond Pearce
 * Date of edits: 4/3/2026
 **********************************************************************************************************************************************************
...
module i2c_master(input             i_clk,              //input clock to the module @100MHz (or whatever crystal you have on the board)
                  ...
                  /** I2C Lines **/
                  input              scl,              //i2c clck line, output by this module, 400 kHz
                  output             scl_pulldown,           //when 1, drive scl with scl_o, when 0, set scl to high impedance (resistors will pull it high)
                  input              sda,              //i2c data line, set to 1'bz when not utilized (resistors will pull it high)
...
reg        reg_sda_o;
reg        reg_sda_o_en; //if 1, then drive sda with reg_sda_o, else set sda to high impedance (resistors will pull it high)
        ...
        case(state)
            ...
            RESTART: begin
                if(!scl_curr & scl_prev) begin
                    ...
                    reg_sda_o_en <= 1'b1;           //Drive the line
                end
            ...
            SLAVE_ADDR: begin
                ...
                if(byte_sent & cntr[0]) begin
                    ...
                    reg_sda_o <= 1'b0;                      //release sda line
                    reg_sda_o_en <= 1'b0;                   //disable sda output
                    cntr <= 0;
                ...
                if(byte_sent & cntr[0]) begin
                    ...
                    if(scl_is_low) begin
                        if(clk_i2c_cntr == DATA_HOLD_TIME) begin
                            ...
                            reg_sda_o <=  byte_sr[7];               //send MSB
                            reg_sda_o_en <= 1'b1;                    //enable sda output
                            byte_sr <= {byte_sr[6:0], 1'b0};        //shift out MSB
                        end
                    end
                end
...
always@(negedge i_clk or negedge reset_n) begin
    if(!reset_n) begin
        ...
        sda_curr <= {sda_curr[0], sda};  //2 flip flop synchronization chain
        sda_prev <= sda_curr[1];
        scl_curr <= scl;
        scl_prev <= scl_curr;
    end
...
//inout cannot be reg
assign scl_pulldown = (~clk_i2c) & en_scl;     //the line will be pulled up to VCC so 1'bz is high
assign sda_pulldown = (~reg_sda_o) & reg_sda_o_en;     //the line will be pulled up to VCC so 1'bz is high
endmodule